...
vi.mock('./teledex', () => ({
  teledex: {
    ...
    // emit() in logger.ts gates on teledex.shouldCapture() before calling capture(); the upstream mock
    // omitted it, so the call threw & was swallowed by emit()'s try/catch (capture never ran). stub it true.
    shouldCapture: () => true,
  },
}));
