...
export const useBattlePresets = (
  ...
  // (also skip for Champions formats, which the pkmn APIs don't publish — upstream behavior.)
  const customFormat = detectPokeathlonModFormat(format);
  const shouldSkipFormats = shouldSkipAny || randoms || customFormat || champions || !downloadSmogonPresets;
  const shouldSkipFormatStats = shouldSkipAny || randoms || customFormat || champions || !downloadUsageStats;
  ...
  const presets = React.useMemo<CalcdexPokemonPreset[]>(() => {
    ...
    const output = [
      ...
      // Pokéathlon: the server's own usage sets (incl. fusions) + bundled community sample sets
      ...(pokeathlonPresets || []),
  ...
  const usages = React.useMemo<CalcdexPokemonPreset[]>(() => (
    randoms
      ...
      // (Pokéathlon: also fold in the server's own usage sets)
